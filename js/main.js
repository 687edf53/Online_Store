...
let distance = (window.innerWidth - (shopContainer.offsetWidth + cart.offsetWidth))
...
function showData() {
  ...
  for (let i = 0; i < cartContent.children.length; i++) {
    cartContent.children[i].getElementsByClassName('cart-remove')[0].addEventListener('click', removeCartItem)
    cartContent.getElementsByClassName('cart-quantity')[0].addEventListener('input', quantityChanged)
  }
}
showData()
